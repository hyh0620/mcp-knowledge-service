...
JSONL 中的 `collection` 当前用于标记数据集预期对应的知识集合；执行器以 CLI 的 `--collection` 作为实际运行配置。运行时必须保证两者一致。当前不支持在同一批评估中按用例动态切换多个 Collection。
...
这是小型受控语料上的独立检索回归结果，不是生产准确率或通用 Benchmark。已提交的去敏快照记录 dataset SHA256、corpus version、模型标识、检索参数、逐样本排名和延迟，并由验证脚本重新计算摘要。

本次结果的可验证快照：
[`verified_20260724_9017a43.json`](../eval/snapshots/verified_20260724_9017a43.json)。
